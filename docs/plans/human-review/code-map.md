...
5. **2b2 Coupling sections** (only when target names two refs, SKILL.md:254-287):
   `structure.py`, `complexity.py`, `coupling.py` (order-free) then `layers.py` (needs
   `coupling.json`) → each JSON, then `scripts/sections.py --kind {layers,coupling,structure}
   --format md` → `section-<kind>.md`.
6. **2b3 Symbol-delta** (same two-ref condition, SKILL.md:289-322):
   ...
   (html only) → `section-symbols.html`.
...
8. **2d Walkthrough**: `scripts/walkthrough.py --format html` and `--format md`
   ...
   optionally `coupling.json` and `complexity.json`.
...
pasted not regenerated), `references/graphs.md` (layers/coupling/structure/symdelta behavior
...
`body` (render.py:142-206) is a Python list of literal HTML strings built up procedurally:
facts strip, `What changed`, `Flow` (mermaid), `How it works`, the pasted `explorer`/`symbols`
HTML section files verbatim, the pasted `walkthrough` HTML verbatim, then a footer `div`. There
...
Function signatures (render.py:135-136, 232, 289-317):
...
- `files` = `{path: {"hunks": [...], "added": int, "removed": int}}`, produced by
  `validate_analysis.parse_hunks(diff_text)` (imported at render.py:45) — i.e. render.py
  re-derives file/line stats straight from `raw.diff`, not from `analysis.json`.
- `explorer`/`walkthrough`/`symbols` are raw HTML **strings** (already-rendered section files),
  pasted with `.rstrip("\n")`, not parsed or re-escaped (render.py:180-199).
...
- `complexity` = parsed `complexity.json` dict, only used via `complexity.summary_line()`
  (render.py:43, 148-150) for one fact-strip chip.
...
**`sections.py`** consumes `coupling.json` / `structure.py`'s JSON / `layers.json` /
`symdelta.json` and turns them into markup strings (md or html), never JSON out. Entry points:

```python
def render(kind, data, fmt):          # sections.py:224, for layers/coupling/structure
def render_symbols(data):             # sections.py:715, for symdelta's kind=symbols, html only
```

`data` here is exactly what `coupling.py`/`structure.py`/`layers.py`/`symdelta.py` emit — see
§6 for their shapes. `sections.py` does have an internal normalized intermediate,
`edges_of(data)` (sections.py:100-114), which flattens the three analysers' differing edge
shapes into `[(src, dst, state, weight)]` tuples — the closest thing to a shared schema, but
it's a transient in-memory list, never serialized, and used only to compute `numbers()`
(caption stats), not for drawing.

**`walkthrough.py`**'s entry points:

```python
def render_html(groups, files, by_path, open_count=DEFAULT_OPEN, links=None, complexity=None,
                 renames=None):                                            # walkthrough.py:326
def render_md(groups, files, by_path, hunks="notes", links=None, max_chars=DEFAULT_MAX_CHARS,
              complexity=None, renames=None):                              # walkthrough.py:434
def story(order, files, groups, coupling):                                 # walkthrough.py:130
```
...
with `caller_depth()` (mechanical, from `coupling.json`) and `interest_rank()` (mechanical,
...
`by_path` = `{path: analysis_file_entry}` (walkthrough.py:585), i.e. a dict view directly over
...
form of the mechanical parts (`files` stats, `groups` resolved order, coupling-derived depth).
...
Full file read. `validate(diff_text, analysis, recap=False)` (validate_analysis.py:223-321) is
...
(validate_analysis.py:94-156, the single owner of diff-parsing reused by render.py and
...
- every file's `role` non-blank and not a bare banned verb (`_is_bare_verb_role`,
  validate_analysis.py:170-176, `BANNED_ROLE_WORDS` at 88-91);
...
- a non-blank hunk `note` must share a tokenized identifier with that hunk's own added/removed
  lines (`_identifier_pieces`, validate_analysis.py:179-200, camelCase/snake_case aware,
  stopword-filtered);
- global `EMPTY_NOTE_FLOOR = 0.8` (validate_analysis.py:77, checked at 207-220): more than 80%
  of hunks with empty notes fails the whole gate;
- `groups` (if present) reference only real diffed paths, no path claimed twice
  (`_validate_groups`, validate_analysis.py:324-351);
- with `--rendered`, every diffed path must appear somewhere in the rendered text
  (`check_rendered`, 354-357) and every generated section's `<!-- visual-diff:<kind> -->`
  marker must survive into the rendered output (`check_sections`, 360-375).
...
## 6. One-liner survey: coupling/structure/layers/complexity/symdelta/links/fanout

| Script | Diff as input? | Per-file or whole-set | Input | Output |
|---|---|---|---|---|
| `coupling.py` | No — takes `--base`/`--head` git refs directly, diffs them itself via `git show`/`list_repo_files` | Whole-set restricted to touched files (`--files` optional filter); entry `analyse(repo, base, head, files)` at coupling.py (main at line 704) | two refs (+ optional path filter) | JSON: `added`/`removed`/`unchanged` edge lists, `mermaid`, `note`, `skipped` |
| `structure.py` | No — same `--repo/--base/--head[/--files]` shape, no `--diff` flag (structure.py:881-884) | Whole-set restricted to touched files; walks classes/functions inside each | two refs | JSON: `added_notests`/`removed_notests`/`unchanged_notests` edges, `structures_added/removed`, `mermaid`(+`_notests`), `ids`, `truncated`, `skipped`, `note` (structure.py:860-875) |
| `layers.py` | No — `--head` only (default `HEAD`), plus `--coupling <coupling.json>` (layers.py:305-307) | **Whole repo**, the only one of the three that reads files the diff never touched (`analyse(repo, head, coupling_path)`, layers.py:220) | one ref + coupling.json | JSON: module-folded mermaid map, touched-module markers |
| `complexity.py` | **Yes**, `--diff` required (complexity.py:370, "for the file list") | Per-file, but scoped to "the worst touched function," not a file total (`analyse(repo, base, head, paths, hunk_ranges, diff_text)`, complexity.py:242) | two refs + raw.diff | JSON: per-file `{jump, peak}` complexity deltas; `summary_line()` renders the fact-strip chip |
| `symdelta.py` | No explicit `--diff` flag — `--repo/--base/--head` only (symdelta.py:882-884); resolves its own diff internally | Two tiers: Go = whole-repo extraction (`go/packages`, cheap); TS/Python/Rust = LSP, scoped only to files the diff changed (`outgoingCalls`/`incomingCalls`) | two refs | JSON via `build_graph()` (symdelta.py:671): `nodes`, `edges`, `moved`, `language`, `reason` |
| `links.py` | **Yes**, `--diff` required (links.py:116-118) | Per-file/per-hunk, keyed to the diff's own hunks | repo path + raw.diff + head ref (+ optional `--pr`) | JSON via `build(repo, diff_text, head, pr)` (links.py:78): `repo_url`, `pr_url`, `head_sha`, `head_pushed`, `files[]` with `diff_url`/`blob_url`/per-hunk `url` |
...
Only `complexity.py`, `links.py`, and `fanout.py` take the diff directly; `coupling.py`,
`structure.py`, and `symdelta.py` take two git refs and diff internally (so they'd need re-running
whole, not incrementally, unless refactored); `layers.py` is the outlier needing the **whole
repo tree**, not just the diff.
...
- `links.py:75` — `"diff-" + hashlib.sha256(path.encode()).hexdigest()`, used purely to build a
  unique HTML anchor id, not a change-detection fingerprint.
- `structure.py:167` — `@functools.lru_cache(maxsize=None)` on `_get_parser` (per-process
  parser-instance memoization, not cross-run).
...
`links.py:78`'s `build()` return shape) holds: `repo_url`, `pr_url`, `head_sha`, `head_pushed`
...
14 test files under `scripts/`, 7228 lines total, all "Self-check for X.py. Assert-based, no
...
| `test_layers.py` | 155 | whole-repo module folding, touched-module marking, using throwaway git repos |
| `test_links.py` | 160 | URL building, sha256 anchor ids, `head_pushed` gating |
| `test_structure.py` | 302 | Python structure-graph parsing/edges via throwaway repos |
| `test_fanout.py` | 313 | split/merge batching, rename-map canonicalization |
| `test_structure_generic.py` | 424 | non-Python (tree-sitter generic) structure parsing |
...
| `test_render.py` | 627 | **the exact string-interpolation contract**: both placeholders replaced, escaping-before-backtick-promotion order, mermaid `flowchart LR`→`TB` rewrite, section-note placement, byte-for-byte pasting of section files, overflow-warning math tied to `walkthrough.py`'s own floor marker |
| `test_coupling.py` | 747 | import-graph edge resolution across Python/JS/Go, added/removed/unchanged classification |
| `test_sections.py` | 944 | mermaid diagram generation (packages/symbols levels, label wrapping, escaping) for all four kinds |
| `test_walkthrough.py` | 971 | **the line-numbering DOM contract** (`test_every_diff_line_is_its_own_span_with_the_right_class`, `test_line_kinds_add_up_to_the_spans_the_header_declares`, `test_html_carries_the_markup_the_comment_code_reads` — these three assert exactly the span-class contract the template's JS in §4 depends on), group/reading-order derivation from `coupling.json`, size-budget demotion, rename handling |
...
- `test_links.py`, `test_layers.py`, `test_coupling.py`, `test_structure*.py`,
  `test_complexity.py`, `test_symdelta.py`, `test_fanout.py`, `test_sections.py` mostly pin
  each analyser's own JSON output shape and are orthogonal to the human-review rendering
  change — they'd only need touching if incremental/hunk-hash regeneration changes their CLI
  contracts (e.g. adding a `--since-hash` filter).
